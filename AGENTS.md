...
## 11. 작업 진행 프로토콜 (Task Execution Protocol)
...
- 이해한 내용을 **요약하여 확인 요청**한다(MUST).
- 사용자가 "맞아/좋아/진행해"라고 명시할 때까지 명확화를 계속한다.
- 합의 없이 실행을 시작해서는 안 된다(MUST NOT).
...
- 사용자가 계획을 **승인한 후에만** 실행을 시작한다(MUST).
- 각 Step 완료 시 체크박스를 업데이트한다 (`[ ]` → `[x]`).
- 계획에 없던 변경이 필요하면 **즉시 중단**하고 계획 수정 승인을 받는다.

### 예외: 단순 작업

다음 조건을 **모두** 만족하는 경우, Phase 1~3을 생략하고 바로 실행할 수 있다:

- 사용자 요청이 명확하고 구체적임
- 영향 범위가 단일 파일 또는 명백히 한정됨
- 아키텍처/Public API 변경이 아님
