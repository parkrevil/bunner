...
- **이 문서는 CLI 설치/운영 정책과 런타임과의 관계를 정의한다.**
...
| 영역      | 명령어                       |
| --------- | ---------------------------- |
| 전체 검증 | `bun run verify`             |
| 타입 체크 | `bun run tsc`                |
| 린트      | `bun run lint`               |
| 테스트    | `bun test`                   |
| 아키텍처  | `bun run architecture:check` |

## CLI 검증 체크리스트

- [ ] CLI가 프레임워크 런타임 패키지에 의존하지 않는가?
- [ ] CLI가 `@bunner/common`에만 의존하는가?
- [ ] AOT 산출물이 결정적인가 (동일 입력 → 동일 결과)?
- [ ] 순환 의존성이 없는가?
- [ ] 해석/파싱 실패가 명확히 보고되는가?
- [ ] `bun run verify`가 통과하는가?
