...
pub fn validate_regex_safe(re: &str) -> bool {
    ...
    if basic_bads.iter().any(|b| re.contains(b)) {
        return false;
    }
    ...
    if re.contains(".{1,1000}") || re.contains(".{1000,}") || re.contains(".{1000}") {
        return false;
    }
...
) -> Result<Regex, InsertError> {
    if RegexParser::new().parse(norm_src).is_err() {
...
fn evict_one(cache: &DashMap<String, (Regex, u64)>) {
    ...
    for (i, e) in cache.iter().take(8).enumerate() {
        let (_, tick) = e.value();
        if *tick < oldest_tick || i == 0 {
            oldest_tick = *tick;
            oldest_key = Some(e.key().to_string());
        }
    }
