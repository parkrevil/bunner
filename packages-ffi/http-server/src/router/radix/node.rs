...
/// Static children map wrapper for readability and future swapping
pub(super) type StaticMap = FastHashMap<String, Box<RadixNode>>;

/// Radix tree node. Holds static children, pattern children, routes, and compression info.
...
impl RadixNode {
    /// Borrow static child by normalized key if present.
    pub(super) fn get_static_ref(&self, key: &str) -> Option<&RadixNode> {
        if let Some(n) = self.static_children.get(key) { return Some(n.as_ref()); }
    ...
    /// Descend to/create static child by normalized key, promoting to map as needed.
    pub(super) fn descend_static_mut(&mut self, key: String) -> &mut RadixNode {
        ...
        self.static_children.entry(key).or_insert_with(|| Box::new(RadixNode::default())).as_mut()
    }

    /// Rebuild first-literal index for `pattern_children` after insertion or reordering.
    pub(super) fn rebuild_pattern_index(&mut self) {
        ...
        for (idx, (pat, _)) in self.pattern_children.iter().enumerate() {
            if let Some(SegmentPart::Literal(l0)) = pat.parts.first() {
                let entry = self.pattern_first_literal.entry(l0.clone()).or_insert_with(|| SmallVec::new());
                entry.push(idx);
            }
        }
    }

    /// Recompute cached pattern scores aligned with `pattern_children`.
    pub(super) fn rebuild_pattern_meta(&mut self) {
        self.pattern_scores.clear();
        self.pattern_scores.reserve(self.pattern_children.len());
        for (pat, _) in self.pattern_children.iter() { self.pattern_scores.push(pattern_score(pat)); }
    }

    /// Return candidate pattern indices whose first literal is a prefix of `comp`.
