...
fn compress_node(n: &mut RadixNode) {
    if let Some((_, c)) = n.pattern_children.first_mut() { compress_node(c.as_mut()); }
    if !n.static_children.is_empty() {
        for (_, v) in n.static_children.iter_mut() { compress_node(v.as_mut()); }
    } else {
        for v in n.static_vals.iter_mut() { compress_node(v.as_mut()); }
    }

    if n.fused_edge.is_some() { return; }
    if !can_compress_here(n) { return; }

    let (mut edge, mut child) = if !n.static_children.is_empty() {
        if n.static_children.len() != 1 { return; }
        let (k, c) = n.static_children.drain().next().unwrap();
        (k, c)
    } else if !n.static_keys.is_empty() {
        if n.static_keys.len() != 1 { return; }
    ...
    loop {
        let terminal = child.routes.iter().any(|k| *k != 0) || child.wildcard_routes.iter().any(|k| *k != 0);
        if child.pattern_children.is_empty() && !terminal {
            if !child.static_children.is_empty() && child.static_children.len() == 1 {
                ...
                edge.push('/'); edge.push_str(&k2); child = c2; continue;
            }
