...
describe('Logger', () => {
  describe('Initialization', () => {
    describe('constructor', () => {
      ...
      test('throws error when dlopen fails', async () => {
        await mock.module('bun:ffi', () => ({
          dlopen: () => {
        ...
        expect(() => createLogger()).toThrow(
          'Failed to initialize Logger: dlopen failed',
        );

        await restoreMocks();
      });

      test('throws error when resolveRustLibPath fails', async () => {
        await mock.module('@bunner/core', () => ({
          encodeCString: encodeCStringSpy,
          resolveRustLibPath: () => {
        ...
        expect(() => createLogger()).toThrow(
          'Failed to initialize Logger: resolveRustLibPath failed',
        );

        await restoreMocks();
      });

      test('handles dlopen returning invalid symbols', async () => {
        await mock.module('bun:ffi', () => ({
          dlopen: () => ({
            symbols: {
              init: undefined,
      ...
      test('handles dlopen returning null symbols', async () => {
        await mock.module('bun:ffi', () => ({
          dlopen: () => ({
            symbols: null,
          }),
    ...
    describe('getInstance', () => {
      ...
      test('handles multiple getInstance calls after error', async () => {
        resetSingleton();
        calls.dlopen.length = 0;
      ...
      const testLogMethod = (method: string, level: number) => {
        describe(method, () => {
          ...
          test('calls encodeCString with message', () => {
            ...
            expect((encodeCStringSpy as any).mock.calls.length).toBe(
              before + 1,
            );
            expect((encodeCStringSpy as any).mock.calls[before][0]).toBe(
              message,
            );
          });
          ...
          test('passes encoded message to log', () => {
            ...
            const lastCall = (logMessageSpy as any).mock.calls[
              (logMessageSpy as any).mock.calls.length - 1
            ];
