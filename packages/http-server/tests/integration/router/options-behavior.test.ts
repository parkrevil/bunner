...
describe('RadixRouter :: options', () => {
  ...
  describe('method compatibility options', () => {
    it('should fall back from HEAD to GET when enabled', () => {
      let getKey!: RouteKey;
      const router = buildRouter(builder => {
        getKey = builder.add(HttpMethod.Get, '/compat/head') as RouteKey;
      });

      const headMatch = router.match(HttpMethod.Head, '/compat/head');
      expect(headMatch?.key).toBe(getKey);
    });

    it('should not fall back from HEAD when disabled', () => {
      const router = buildRouter(
        builder => {
          builder.add(HttpMethod.Get, '/compat/strict');
        },
        { headFallbackToGet: false },
      );

      expect(router.match(HttpMethod.Head, '/compat/strict')).toBeNull();
    });

    it('should synthesize OPTIONS responses with allowed methods metadata', () => {
      const router = buildRouter(builder => {
        builder.add(HttpMethod.Get, '/compat/options');
        builder.add(HttpMethod.Post, '/compat/options');
      });

      const optionsMatch = router.match(HttpMethod.Options, '/compat/options');
      expect(optionsMatch).not.toBeNull();
      expect(optionsMatch?.meta?.source).toBe('auto-options');
      expect(optionsMatch?.meta?.allow).toEqual([HttpMethod.Get, HttpMethod.Post, HttpMethod.Options, HttpMethod.Head]);
    });

    it('should return null for OPTIONS when autoOptions is disabled', () => {
      const router = buildRouter(
        builder => {
          builder.add(HttpMethod.Get, '/compat/options-disabled');
        },
        { autoOptions: false },
      );

      expect(router.match(HttpMethod.Options, '/compat/options-disabled')).toBeNull();
    });
  });
});
