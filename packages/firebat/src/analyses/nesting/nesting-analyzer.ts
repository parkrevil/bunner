...
import type { EarlyReturnAnalysis, EarlyReturnItem, NestingAnalysis, NestingItem } from '../../types';
...
const analyzeFunctionNode = (
  ...
  filePath: string,
  sourceText: string,
): readonly [NestingItem, EarlyReturnItem] | null => {
  ...
  const visit = (node: NodeValue, depth: number): void => {
    ...
    if (nodeType === 'IfStatement' && depth === 0 && isNodeRecord(node)) {
  ...
  const nestingScore = Math.max(0, maxDepth * 3 + decisionPoints - (hasGuardClauses ? 1 : 0));
  const earlyReturnScore = Math.max(0, earlyReturnCount + (hasGuardClauses ? 0 : 1));
  const nestingSuggestions: string[] = [];

  if (maxDepth >= 3 && !hasGuardClauses) {
  ...
  const earlyReturnSuggestions: string[] = [];

  if (!hasGuardClauses && maxDepth >= 2) {
    earlyReturnSuggestions.push('introduce early returns to flatten control flow');
  }

  if (earlyReturnCount >= 4) {
    earlyReturnSuggestions.push('simplify branching to reduce early return count');
  }
  ...
  return [
    ...
    },
    {
      ...
      suggestions: earlyReturnSuggestions,
    },
  ];
};

const collectFunctionItems = (
  files: ReadonlyArray<ParsedFile>,
): readonly [ReadonlyArray<NestingItem>, ReadonlyArray<EarlyReturnItem>] => {
  const nestingItems: NestingItem[] = [];
  const earlyReturnItems: EarlyReturnItem[] = [];
  ...
  for (const file of files) {
    ...
    for (const functionNode of functions) {
      ...
      nestingItems.push(result[0]);
      earlyReturnItems.push(result[1]);
    }
  }

  return [nestingItems, earlyReturnItems];
};
...
const analyzeEarlyReturn = (files: ReadonlyArray<ParsedFile>): EarlyReturnAnalysis => {
  ...
  const [, earlyReturnItems] = collectFunctionItems(files);

  return {
    items: earlyReturnItems,
  };
};

export { analyzeEarlyReturn, analyzeNesting, createEmptyEarlyReturn, createEmptyNesting };
