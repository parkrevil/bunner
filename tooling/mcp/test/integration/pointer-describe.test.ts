...
describe('mcp/pointer_describe', () => {
  it('returns KB_SCHEMA_MISSING when DB is unavailable', async () => {
    ...
    try {
      const res = await client.callTool({ name: 'pointer_describe', arguments: { pointerId: 1 } });
      const sc = (res as any).structuredContent;

      expect(sc?.error?.code).toBe('KB_SCHEMA_MISSING');
      expect(sc?.error?.tool).toBe('pointer_describe');
    } finally {
      await close();
    }
  });

  it('returns INVALID_ARGUMENTS for missing pointerId', async () => {
    const { client, close } = await createIntegrationClient({ envSource: {} });
    try {
      const res = await client.callTool({ name: 'pointer_describe', arguments: {} as any });
      const sc = (res as any).structuredContent;
