...
- 자동화는 기본 브랜치(또는 보호 브랜치)에 **직접 커밋/푸시해서는 안 된다**(MUST NOT). 변경은 PR(또는 동등한 리뷰 워크플로우)로만 제안해야 한다(MUST).
- 자동화는 결정성을 해쳐서는 안 된다(MUST NOT). (툴체인/의존성/출력물은 재현 가능해야 한다.)
...
범위가 불명확하여 중단해야 하는 경우, 다음 형식으로 요청한다:

```text
## 범위 명세 요청

**현재 상황**: [무엇을 하려다가 범위 불명확 발생]
**불명확한 점**:
- [질문 1]
- [질문 2]
...
**현재 상황**: UserService 리팩토링 중 관련 테스트 수정 필요
**불명확한 점**:
- 다른 feature의 통합 테스트도 수정 범위인가?
- 테스트 픽스처 공용화도 진행할까?

**제안**:
- 옵션 A: UserService 유닛 테스트만 수정
- 옵션 B: UserService 관련 모든 테스트 수정
