...
- Diff evidence:
  - Changed files (actual): `packages/firebat/src/arg-parse.ts`, `packages/firebat/src/firebat.ts`, `packages/firebat/src/types.ts`, `packages/firebat/src/report.ts`
- Verification evidence:
  - LOG-VERIFY: `not-run`
- MUST-EVID mapping:
  - MUST-EVID-8: `CLI --only 확장 및 output wiring 완료`
