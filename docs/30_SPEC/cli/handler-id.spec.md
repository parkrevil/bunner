# CLI Handler ID Specification

## 0. 정체성(Identity) (REQUIRED)

| 필드(Field)      | 값(Value)                                 |
| ---------------- | ----------------------------------------- |
| Title            | CLI Handler ID Specification              |
| ID               | CLI-HANDLER-ID                            |
| Version          | v1                                        |
| Status           | Draft                                     |
| Owner            | repo                                      |
| Uniqueness Scope | spec                                      |
| Depends-On       | path:docs/30_SPEC/common/identity.spec.md |
...
| Item                               |
| ---------------------------------- |
| CLI handler identifier conventions |
...
| ------------------------- | ----------------------------------------- | ----------------- |
| source                    | path:docs/30_SPEC/common/identity.spec.md | identity contract |
...
| --------------------- | ------------------------- | ------------------------------- | ------------------------------------- | ------------------------------------------- | ----------------------------------------------- |
| cli-handler-id        | code (AOT)                | string-literal                  | yes                                   | yes                                         | none                                            |

### 3.2 정적 데이터 형상(Static Data Shapes) (REQUIRED)

```ts
export type HandlerId = `${string}:${string}#${string}`;

export type HandlerIdFormat = '<adapterId>:<file>#<symbol>';
```
...
| -------------------- | --------------------------- | --------------- | ----------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| CLI-HANDLER-ID-R-001 | active                      | MUST            | inputs, artifacts, shapes, outcomes | InputKind:cli-handler-id, Artifact:HandlerId, Shape:local:HandlerId, Outcome:OUT-001 | HandlerId matches `<adapterId>:<file>#<symbol>` and each segment is non-empty                                                                                            | build                             |
| CLI-HANDLER-ID-R-002 | active                      | MUST            | shapes, outcomes                    | Shape:local:HandlerId, Outcome:OUT-002                                               | adapterId segment is a non-empty string matching `^[A-Za-z0-9._-]+$`                                                                                                     | build                             |
| CLI-HANDLER-ID-R-003 | active                      | MUST            | shapes, outcomes                    | Shape:local:HandlerId, Outcome:OUT-003                                               | file segment MUST NOT include ":" or "#" and symbol is `<controllerClassName>.<handlerMethodName>` where each segment is non-empty and MUST NOT include ":", "#", or "." | build                             |
...
| ------------------------- | ------------------ | -------------------------- | ---------------------------------- |
| HandlerId                 | schema             | local:HandlerId            | this-spec-only                     |
...
| ------------------ | ------------------------------- | ----------------------------------- | ------------------------- | --------------------------------------------- | ------------------------------------------------- | --------------------------------- |
| HandlerId          | glob                            | docs/30_SPEC/cli/handler-id.spec.md | build                     | n/a                                           | n/a                                               | n/a                               |
...
| -------------------------- | -------------------- | ------------------------ | ---------- | -------------------------------------------- |
| handler id declared        | CLI-HANDLER-ID-R-001 | Artifact:HandlerId       | OUT-001    | HandlerId is mechanically checkable          |
| handler id validated       | CLI-HANDLER-ID-R-002 | Outcome:OUT-002          | OUT-002    | HandlerId adapterId segment is valid         |
| handler id validated       | CLI-HANDLER-ID-R-003 | Outcome:OUT-003          | OUT-003    | HandlerId file and symbol segments are valid |
...
| From              | To Document                               |
| ----------------- | ----------------------------------------- |
| identity contract | path:docs/30_SPEC/common/identity.spec.md |
