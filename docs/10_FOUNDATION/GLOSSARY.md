...
- **Error Filter**: throw로 발생한 예외를 입력으로 받아, 표준 Result로 변환하거나(처리) 다음 단계로 전달(통과)하는 실행 단계이다.
- **Error Filter Chain**: 순서가 있는 Error Filter의 리스트이다. 예외는 체인의 앞에서 뒤로 전달되며, 처리되지 않은 예외는 체인의 후단으로 전달된다.
