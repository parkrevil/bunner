...
| 16  | 사용되지 않는 메타데이터      | 확정   | Low         | Phase 3에서 `methods.version` 필드를 전면 제거(2025-11-16). 라우터 노드 메타데이터가 단순화되어 중복 상태 관리가 사라졌습니다.                                                                |
...
- 사용되지 않는 메타데이터(`methods.version`)는 Phase 3에서 제거 완료. 향후 `router.ts` 책임을 하위 모듈로 분리하는 리팩터링을 계획합니다.

## Phase 3 진행 상황

- 항목 6 부분 캐시 무효화: `packages/http-server/src/router/router.ts`에서 캐시 키 ↔ 경로 인덱스를 유지하면서 겹치는 경로만 무효화하도록 변경했습니다. 정적 라우트 추가 시 정확히 해당 경로/메서드만 제거하고, 동적 라우트는 최초 정적 프리픽스를 기준으로 선별 삭제합니다.
- 항목 7·19 국소 압축: 새로운 정적 노드가 생성되거나 체인이 분할될 때만 `requestCompression`으로 해당 서브트리만 다시 압축하도록 바꿨습니다. `ensureCompressed`는 대기 중인 노드 집합만 처리하므로 전체 트리를 재탐색하지 않습니다.
- 항목 8 와일드카드 접미사: `packages/http-server/src/router/match-walker.ts`에서 접미사를 하나의 소스 문자열+오프셋으로 지연 계산하여 O(n²) 문자열 복제를 제거했습니다.
- 항목 15 미세 최적화: 캐시 키 생성과 정적 미스 기록 시 불필요한 중복 연산을 제거하고, 캐시 루틴에서 메서드 프리픽스를 재활용하도록 정리했습니다.
- 회귀 테스트: `packages/http-server/tests/integration/router/options-behavior.test.ts`에 캐시 보존/무효화 시나리오를 추가해 새 동작을 보장합니다.
